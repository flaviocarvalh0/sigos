...
import { OrdemServicoPeca, OrdemServicoPecaCriacaoPayload } from '../../../Models/ordem-servico/ordem-servico-peca.model';
...
export class PecasOrdemServicoComponent implements OnInit {
  ...
  calcularTotal(): number {
    return this.pecasAdicionadas.reduce((acc, p) => acc + (p.valorTotal || 0), 0);
  }

  getDescricaoPeca(p: OrdemServicoPeca): string {
  return p.nomePeca || this.pecasDisponiveis.find(pd => pd.id === p.idPeca)?.descricao || 'Desconhecida';
}

aoSelecionarPeca(): void {
  const pecaSelecionada = this.pecasDisponiveis.find(p => p.id === this.novaPeca.idPeca);
